...
const checkLowStock = inngest.createFunction(
    { id: "check-low-stock", name: "", triggers: [{ event: "inventory/stock.updated" }] },
    async ({ event, step }) => {
        ...
        await step.run('send-low-stock-email', async () => {
            ...
            if(adminEmails.length === 0) return { skipped: true, reason: "no admin email"};
...
const autoAssignRider = inngest.createFunction({
    ...
    const result = await step.run("assign-rider", async()=> {
        ...
        if(order.deliveryPartnerId) return {skipped: true, reason: "Aleady Assigned"};
